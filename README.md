...
- Cadastro: "Importar um progresso exportado" (o mesmo `#imp-file` de Perfil; `importPanel` é o bloco Substituir/Juntar, usado nas duas telas). Resumo do mês: `monthStats(back)` (minutos, dias, maior sequência e conquistas pela data em `S.badges`), painel em Hoje nos sete primeiros dias do mês (`S.lastMonthSeen`, `month-seen`), "Este mês" em Evolução → Números e `month-share` por `shareText` (navigator.share ou cópia).
- Aprendizado motor: treino intercalado (`S.interleave`: '' segue o nível, ligado do 3 em diante; `interleave(out)` divide blocos de 4 min ou mais em duas rodadas, a segunda com id `x.2`, e "Onde vai o tempo" soma pelo id base), descanso curto nas Mãos (`MAO.rest`: os motores `sgSched` e `grStep` silenciam nas janelas de 10 s a partir de `SG.t0`/`GR.at0`), andamento variável no Tocar junto (`BAT.vary`, `base`; `grStep` sorteia o BPM a cada volta e `grStop` devolve a base) e ensaio mental (`MI`, `mentalPanel`: 5 s olhando as formas, 25 s imaginando, no bloco de trocas de Hoje e em Treinar, Trocas).
- Memória: "Memorizar" na cifra (`s.memo` 0 a 4; `memoHide` troca uma fração determinística dos botões `cif-ch` por "?" com classe `hid`, o toque revela; `s.playsMemo` conta passagens de cabeça e a dica antes de "Marcar como dominada"), palpite antes de aprender (`chordNew`, `S.pre`, `.prebox` no bloco de trocas; `pre-try` abre Descobrir com `CFI.pre`, `pre-check` compara com `chordIdentify` e chama `revGrade('diag:c')`), revisão espaçada de músicas (`SONG_STEPS`, `songDue`, `s.next`/`s.step`; bloco "Repertório: revisão" com `song-recall`) e batidas na revisão diária (`pat:` em `revCandidates`/`revQuestion`).
- Hábito: intenção de implementação (`S.intent` {after, place}, `intentPanel` no painel do lembrete, `intentText` em Hoje e em `reminderBody`), treino dividido (`S.split`, `S.remind2`; marcadores "Parte 1/Parte 2" em Hoje e um terceiro bloco de notificações em `scheduleReminder`), dica do sono (`sleepTip`, a partir das 20 h) e "Quando você treina" em Evolução (`d.at`, a hora do primeiro bloco concluído no dia, gravada em `blockDone`).
- Feedback e ponto de desafio: em "Conferir pelo som", a primeira leitura estável fica pendente (`CHK.est`, `CHK.pending`) até a pessoa responder "Como soou?" (`chk-est`; `S.chkEst` liga e desliga); `chkClean` registra dias limpos por acorde em `S.chkLog` e marca `S.chords[c]='dominado'` no terceiro dia. Trocas: `tGoal(k)` (recorde + 2, ou 10), mostrada no painel, no bloco de Hoje e na tela final (`T.goal`, `T.hit`). Ouvido: `earLevelUp` também desce um nível com dez respostas abaixo de 50%.
- O erro como alvo: trecho difícil na cifra (`SV.pick='hard'` faz os dois toques seguintes em `cif-ch` gravarem `s.hard` {a,b} e o loop `SV.la/lb`; `hardBox` mostra o contador `s.hardClean`, `hard-ok`/`hard-miss`; `hardBpm` entra em `SV.cfg.bpm` do tocar junto e sobe 4 por passagem limpa; cinco limpas alargam o trecho em `per` acordes por lado até a música inteira). Bloco de ritmo: `arrowsBlock` esconde as setas quando `S.patLog[pid]` tem três dias ou mais (registrados em `blockDone`), com `arrows-show` (sessão, `UIA`) e `S.arrows` (auto, sempre, de cabeça).
- Ouvido com variedade: modalidade `mix` (`earNew` sorteia `md` entre mm, qual, ivl, prog e bat e grava em `E.q.mode`; `earLogAdd` e `earLevelUp` usam o modo sorteado; a tela usa `emode`), vozes variadas em `earVoicing` (forma aberta, `barreAlt` ou oitava acima, com `gap` sorteado) e o plano de imprevisto (`S.intent.ifnot`, `copingTip` em Hoje a partir das 20 h com o botão do treino de 5 minutos).
- Metacognição e feedback: confiança antes de responder na revisão (`RV.conf`, `rv-conf`; as opções e o Conferir ficam travados até a escolha; `revConfLog` acumula em `S.revConf` por nível e `revCalib` resume a sessão no aviso final e em `d.calib`), explicação em cada resposta do ouvido (`earExplain`: `IVL_REF`, `progDegrees`, notas do acorde, células da batida) e "feito por hoje" (tela final das trocas com o próximo par de `pairSuggest`, `t-pair` volta ao começo; `#chk-done` no Conferir pelo som quando `S.chkLog` tem o dia de hoje).
- Geração e exemplos que somem: modalidade `fill` do ouvido (`E.q.blanks`, `E.q.sel`, `fill-cell` cicla ↓ ↑ x ·, `fill-check` compara com as células e entra em `earLevelUp`), modalidade `next` (`earNew` escolhe um tom cujo campo tem quatro acordes na biblioteca, uma progressão de `HF_PROGS.maj` e toca os três primeiros; a explicação vem de `earExplain` em graus) e "Memorizar a letra" (`cifraParse` envolve as linhas de letra em `i.ly`; `lyrHide` esconde uma fração determinística com a primeira palavra de pista; `ly-show` revela; `s.lyr` de 0 a 3).
- Foco, autonomia e reflexão: dicas de foco externo (`FOCUS`, `focusTip` por dia e bloco, classe `.foco`), extra do dia (`extraOptions`: par mais lento de `pairSuggest`, acorde vencido de `revQueue`, batida do nível; `S.extra[k]` e `withExtra` acrescentam o bloco `xtra` depois de `interleave`; `xtra-pick`/`xtra-clear`; blocos com `chk` mostram "Conferir pelo som" e `arrowsHide` força as setas escondidas) e diário guiado (`d.easy`, `d.stuck` no painel do diário; `yesterdayStuck` em Hoje).
