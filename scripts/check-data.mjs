...
D.PATTERNS.forEach(p => {
  ...
  p.cells.forEach(c => { if (!['', '↓', '↑', 'x', 'B', 'P', 'I', 'M', 'A'].includes(c)) err(`batida ${p.id}: célula desconhecida "${c}"`); });
...
D.ROAD.forEach((weeks, li) => {
  ...
  weeks.forEach((W, wi) => {
    ...
    if (W.scale && !D.SCALES.some(s => s.id === W.scale.sc)) err(`${tag}: escala desconhecida ${W.scale.sc}`);
  });
});
